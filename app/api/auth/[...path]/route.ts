...
async function handleLogin(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!usn || !password) return NextResponse.json({ detail: "USN and password required" }, { status: 400 });

    const { data: students } = await supabase.from("students").select("*").eq("usn", usn.trim().toUpperCase()).limit(1);
    ...
    if (!student) {
      ...
      const { data: newStudent, error } = await supabase.from("students")
        ...
        .select().single();
      if (error || !newStudent) return NextResponse.json({ detail: "Registration failed: " + (error?.message || "Unknown") }, { status: 500 });
      ...
      const { data: statusData } = await supabase.from("exam_status").select("status").eq("student_id", student.id).single();
      if (statusData?.status === "submitted") return NextResponse.json({ detail: "You have already submitted this exam." }, { status: 403 });
    }
